...
if (state == SJEND) {
    ...
    if (sjball == 0) {
        ...
        if (statetime == 10) {
            if (facing == RIGHT) {
                if (global.currentsuit == 0) sprite_index = sJumpRight;
                if (global.currentsuit == 1) sprite_index = sVJumpRight;
                if (global.currentsuit == 2) sprite_index = sGJumpRight;
                image_index = 0;
                image_speed = 0.5;
            }
            if (facing == LEFT) {
                if (global.currentsuit == 0) sprite_index = sJumpLeft;
                if (global.currentsuit == 1) sprite_index = sVJumpLeft;
                if (global.currentsuit == 2) sprite_index = sGJumpLeft;
                image_index = 0;
                image_speed = 0.5;
            }
        }
        if (statetime == 28) {
            if (facing == RIGHT) {
                if (global.currentsuit == 0) sprite_index = sSJumpStart;
                if (global.currentsuit == 1) sprite_index = sVSJumpStart;
                if (global.currentsuit == 2) sprite_index = sGSJumpStart;
                image_index = 2.5;
                image_speed = -0.15;
            }
            if (facing == LEFT) {
                if (global.currentsuit == 0) sprite_index = sSJumpStartL;
                if (global.currentsuit == 1) sprite_index = sVSJumpStartL;
                if (global.currentsuit == 2) sprite_index = sGSJumpStartL;
                image_index = 2.5;
                image_speed = -0.15;
            }
        }
...
if (state == SAVINGSHIP) {
    ...
    if (statetime == 5) {
        facing = 0;
        if (global.currentsuit == 0) sprite_index = sFront;
        if (global.currentsuit == 1) sprite_index = sVFront;
        if (global.currentsuit == 2) sprite_index = sGFront;
        image_speed = 0.1;
    }
    if (statetime < 48) y += 1;
...
if (state == WATERJET) {
    ...
    xFric = 1;
    if (statetime == 0) {
        if (global.currentsuit == 0) sprite_index = sMorphBall;
        if (global.currentsuit == 1) sprite_index = sVMorphBall;
        if (global.currentsuit == 2) sprite_index = sGMorphBall;
        image_speed = 0;
        image_index = 4;
        xVel = 0;
        xAcc = 0;
...
if (state == GRABBEDGAMMA) {
    ...
    invincible = 5;
    canturn = 0;
    if (global.currentsuit == 0) global.playerhealth -= 0.8;
    if (global.currentsuit == 1) global.playerhealth -= 0.5;
    if (global.currentsuit == 2) global.playerhealth -= 0.3;
...
if (global.watertype == 1 && inwater && global.currentsuit < 2) {
    ...
    burning = 1;
    if (global.currentsuit == 0) global.playerhealth -= 0.1;
    if (global.currentsuit == 1) global.playerhealth -= 0.05;
    if (global.playerhealth <= 0) with (oControl) event_user(1);
} else burning = 0;
if (plantdrain > 0) {
    plantdrain -= 1;
...
if (monster_drain > 0) {
    ...
    dash = 0;
    speedboost = 0;
    if (global.currentsuit == 0) global.playerhealth -= 2;
    if (global.currentsuit == 1) global.playerhealth -= 1;
    if (global.currentsuit > 1) global.playerhealth -= 0.5;
    if (global.playerhealth <= 0) with (oControl) event_user(1);
...
if (sfx_isplaying(sndSBallLoop) && sball == 0) sfx_stop(sndSBallLoop);
if (sfx_isplaying(sndLavaLoop) && burning == 0) sfx_stop(sndLavaLoop);
if (sfx_isplaying(sndPlantLoop) && plantdrainfx == 0) sfx_stop(sndPlantLoop);
if (sfx_isplaying(sndDrainLoop) && monster_drainfx == 0 && queen_drainfx == 0) sfx_stop(sndDrainLoop);
